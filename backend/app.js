...
function normalizeLanguage(value) {
  ...
  if (['es', 'es-mx', 'es_es', 'es-es', 'es_ar', 'es-co', 'es-cl', 'es-pe'].includes(lang)) return 'es';
  return 'es';
}
...
function classifySummary(matches) {
  const summary = { total: matches.length, spelling: 0, grammar: 0, punctuation: 0, style: 0, other: 0 };
  for (const match of matches) {
    const issueType = String(match.rule?.issueType || '').toLowerCase();
    const category = String(match.rule?.category?.name || '').toLowerCase();
    if (issueType.includes('misspelling') || category.includes('ortograf')) summary.spelling += 1;
    else if (issueType.includes('typographical') || category.includes('puntu')) summary.punctuation += 1;
    else if (issueType.includes('grammar') || category.includes('gram')) summary.grammar += 1;
    else if (issueType.includes('style') || category.includes('estilo')) summary.style += 1;
...
app.get('/', (_req, res) => {
  res.json({ ok: true, message: 'Corrector backend activo' });
});

app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'corrector-backend', languagetool_url: LANGUAGETOOL_URL });
});
...
app.post('/api/check', async (req, res) => {
  try {
    ...
    if (!text) return res.status(400).json({ ok: false, error: 'Debes enviar un texto para analizar.' });
    ...
    return res.json({ ok: true, language, text, correctedText, summary, matches: mappedMatches });
  } catch (error) {
    console.error('Error en /api/check:', error.message);
    return res.status(500).json({ ok: false, error: 'No se pudo analizar el texto en este momento.', details: error.message });
  }
});
...
app.post('/api/correct', async (req, res) => {
  try {
    const text = String(req.body?.text || '').trim();
    const language = normalizeLanguage(req.body?.language);
    if (!text) return res.status(400).json({ ok: false, error: 'Debes enviar un texto para corregir.' });
    ...
    return res.json({ ok: true, language, originalText: text, correctedText, summary, matches: mappedMatches });
  } catch (error) {
    console.error('Error en /api/correct:', error.message);
    return res.status(500).json({ ok: false, error: 'No se pudo corregir el texto en este momento.', details: error.message });
  }
});
