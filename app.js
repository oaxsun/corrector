...
const LANGUAGETOOL_URL = process.env.LANGUAGETOOL_URL || 'http://127.0.0.1:8081/v2/check';
...
function buildRequestBody(text, language = 'es') {
  ...
  params.set('motherTongue', 'es');
  params.set('preferredVariants', 'es');
  return params;
}
  ...
  try {
    ...
    if (text.length > 20000) {
      return res.status(413).json({
        ok: false,
        error: 'El texto es demasiado largo. Límite sugerido: 20,000 caracteres por solicitud.'
      });
    }
